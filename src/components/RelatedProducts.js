  ...
  return (
    <div className="mt-16">
      <div className="flex justify-between items-center mb-8">
        ...
        <Link href={`/category/${category}`} className="text-blue-600 hover:underline flex items-center font-medium">
          View All <ArrowRightIcon className="w-5 h-5 ml-2" />
        </Link>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {products.map((related) => (
          <div key={related.id} className="bg-white p-5 rounded-xl shadow-md border border-gray-200 hover:shadow-lg transition duration-300">
            ...
            <h3 className="font-semibold text-gray-900 text-lg truncate">{related.name}</h3>
            <p className="text-blue-700 font-bold text-xl mt-2 p-1 bg-blue-100 inline-block rounded-lg">${related.price}</p>
            <Link
